...
1. Mount the component with `@zakstam/codex-runtime/convex.config`.
...
4. Start runtime with `createCodexHostRuntime(...)`:
   - omit `persistence` for Codex-authoritative runtime with no external persistence package.
   - pass a persistence adapter implementation when enabling durable replication.
5. Build UI with `@zakstam/codex-runtime-react` hooks.
6. Run `pnpm --filter @zakstam/codex-runtime run doctor:integration` before claiming integration complete.
