...
- 0e888f9: Unify public thread identity contracts on `threadHandle` across host/component boundaries.
...
- 4c50ef9: Unify public thread identity contracts on `threadHandle` across host/component boundaries.
  - `threads.list` now returns `threadHandle` in page rows.
...
- 619887d: Refactor runtime startup to a transport-first contract with explicit thread intent.
...
- 2943cb5: Improve debug-harness verification ergonomics and reduce release-check friction.
...
- 6768f54: Add a canonical bridge lifecycle contract for external consumers.
...
- c99944f: Gate package publishing on successful `codex-local-component` CI runs on `main`, while keeping manual dispatch available for maintainers.
...
- 50438ef: Make thread-scoped host reads safe-by-default and add strict query aliases.
...
- 0d1ce02: Fix a Node ESM runtime import crash when importing `@zakstam/codex-runtime/protocol`.
...
- 0bd951c: Strengthen `createCodexHost` wrapper contract typing so incompatible wrapper signatures are rejected at compile time instead of degrading endpoint contracts.
  ...
  Harden CI by validating `@zakstam/codex-runtime` with the correct package filter and adding a tauri host/type contract gate when boundary files change.
...
- 2947196: Add explicit `returns` validators to public component functions, export `./_generated/component.js` and `./test` from `@zakstam/codex-runtime`, and enforce validator coverage in CI with `check:component-function-validators`.

## 0.16.0

### Minor Changes

- 4ebb562: Remove mode-based host thread wiring and adopt a single-path runtime-owned thread contract.
...
- dddc03c: External API simplification phase 2:
  ...
  - `createConvexPersistence` factory (historical; superseded by adapter extraction to `@zakstam/codex-runtime-convex`)
...
- 3fea5e3: Add a package-owned Tauri host adapter export at `@zakstam/codex-runtime-bridge-tauri`.
...
- 9c9b4ce: Simplify actor-locked host wiring by adding guarded runtime-owned endpoint helpers.
  - Add `defineGuardedRuntimeOwnedHostEndpoints(...)` to `@zakstam/codex-runtime/host/convex` so consumers can apply mutation/query actor guards once.
...
- b3c5b87: Harden linting coverage and expose lint scripts across the published package and app surfaces.
  ...
  - Add `lint` scripts to `packages/codex-runtime` and `apps/examples/tauri-app`.
...
- 4110cf7: Add a package-scoped `coverage` script for local coverage runs:
  - Add `coverage` to `packages/codex-runtime/package.json`:
    `pnpm run build && NODE_V8_COVERAGE=coverage node --test --experimental-test-coverage test/*.test.mjs`
  - Lets contributors run `pnpm --filter @zakstam/codex-runtime run coverage` without CI wiring.
...
- b716998: Improve external consumer onboarding docs in the published package:
  - Keep `packages/codex-runtime/README.md` as the human entrypoint.
  - Keep `packages/codex-runtime/LLMS.md` as the LLM-only routing manifest.
...
- 6418730: Document that `@zakstam/codex-runtime` is now in alpha and ready for active testing (still not production ready), and update release safety allowlists so CI prechecks stay green after recent refactors.
...
- 3634658: Add full app-server thread lifecycle support to the package runtime and helpers.
  ...
  - Expand `@zakstam/codex-runtime/client` thread helper exports (`createThread`, `resolveThread`, `resumeThread`, `resolveThreadByThreadHandle`, `getThreadHandleMapping`, `listThreads`).
...
- 78b7fc7: Add shared host Convex wrapper primitives and validators to reduce duplicated `convex/chat.ts` logic across example/verification apps.
  ...
  Add a Node-safe host entrypoint (`@zakstam/codex-runtime/host/convex`) so Convex server files can import host helpers without pulling Node runtime bridge code into Convex bundling.
...
- ce21324: Refactor sync APIs and consumer helpers for clearer ingest/replay semantics.
  ...
  - Add typed `@zakstam/codex-runtime/app-server` entrypoint
...
- 0966382: Migrate package scope from `@convex-dev/codex-runtime` to `@zakstam/codex-runtime` and update all consumer imports, docs, and release verification checks.
...
- 806e57f: Initial release of `@convex-dev/codex-runtime` with local Codex bridge,
