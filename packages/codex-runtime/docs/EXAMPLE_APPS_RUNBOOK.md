...
- Package `build` scripts are non-destructive (no implicit `clean`) so parallel app terminals do not remove `dist` files mid-startup.
- Use explicit `pnpm run clean` only when intentionally resetting build artifacts.
...
```bash
pnpm --filter codex-runtime-tauri-example run typecheck
```
