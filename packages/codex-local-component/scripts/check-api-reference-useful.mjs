...
function parseSectionsFromMarkdown(text) {
  ...
  for (const line of text.split(/\r?\n/)) {
    const sectionMatch = line.match(
      /^##\s*`@zakstam\/codex-local-component\/([^`]+)`/,
    );
    if (sectionMatch) {
      if (current) {
        sections.push(current);
      }
      const literal = `@zakstam/codex-local-component/${sectionMatch[1]}`;
      const moduleName = literal.startsWith("@zakstam/codex-local-component/")
        ? literal.slice("@zakstam/codex-local-component/".length)
        : ".";
    ...
    if (line.startsWith("## `@zakstam/codex-local-component`")) {
      if (current) {
        sections.push(current);
      }
      current = { moduleName: ".", apis: new Set() };
