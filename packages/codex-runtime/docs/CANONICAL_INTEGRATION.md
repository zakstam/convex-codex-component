...
- omit `persistence` for no external persistence dependency.
- pass a persistence adapter to enable durable replication.

## Steps

1. Choose persistence approach:
   - core-only runtime: omit `persistence` and skip Convex host wiring.
   - Convex persistence: continue with steps 2-4 and pass a Convex adapter.
2. (Convex only) Mount the component in `convex/convex.config.ts` using `@zakstam/codex-runtime/convex.config`.
3. (Convex only) Define host definitions in `convex/chat.ts` with `defineCodexHostDefinitions(...)` from `@zakstam/codex-runtime-convex/host`.
4. (Convex only) Export explicit Convex `mutation/query` wrappers from `convex/chat.ts`.
5. Start runtime with `createCodexHostRuntime(...)` from `@zakstam/codex-runtime/host`.
6. Build UI with hooks from `@zakstam/codex-runtime-react`.
7. (Convex only) Run `chat.validateHostWiring` during startup (`{ actor, conversationId? }`).
...
- Prefer `resolveActorFromAuth(ctx, requestedActor?)` from `@zakstam/codex-runtime-convex/host` to derive canonical host actors from `ctx.auth.getUserIdentity()`.
...
- Runtime persistence is adapter-first:
  - omit `persistence`: Codex-only authoritative mode (no Convex host wiring required).
  - provide `persistence`: Codex-authoritative runtime with adapter-backed projection.
...
- `importLocalThreadToPersistence(...)` fail-closes with `E_PERSISTENCE_DISABLED` when configured adapter does not support durable sync.
...
```tsx
import { CodexProvider, useCodex } from "@zakstam/codex-runtime-react";
