...
- Bridge command wiring is schema-first from `bridge/command-contract.json`.
- Treat `bridge/command-contract.json` as the single source of truth for bridge command names, helper mappings, and Tauri invoke registration.
- Regenerate bridge artifacts with `pnpm run generate:bridge-contract`.
- Validate generated files are up to date with `pnpm run check:bridge-contract`.
- Generated files:
  - `src/lib/tauriBridge.generated.ts`
  - `src-node/bridge-contract.generated.ts`
  - `src-node/bridge-dispatch.generated.ts`
...
- Do not manually edit generated bridge files; update the contract and regenerate.
