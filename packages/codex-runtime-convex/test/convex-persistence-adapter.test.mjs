...
import { createConvexPersistenceAdapter } from "../dist/index.js";
...
test("createConvexPersistenceAdapter.getConversationSyncJob uses query transport", async () => {
  const calls = [];
  ...
  const persistence = createConvexPersistenceAdapter(client, chatApi, { syncJobPollTimeoutMs: 25 });
...
test("createConvexPersistenceAdapter.waitForConversationSyncJobTerminal polls with query transport", async () => {
  const calls = [];
  const chatApi = createChatApi();
  let pollCount = 0;
  const client = {
  ...
  const persistence = createConvexPersistenceAdapter(client, chatApi, {
    syncJobPollMs: 1,
...
test("createConvexPersistenceAdapter.listPendingServerRequests queries by conversation scope", async () => {
  const calls = [];
  const chatApi = createChatApi();
  const client = {
    mutation: async (fn, args) => {
...
test("createConvexPersistenceAdapter.listPendingServerRequests returns [] for missing thread read errors with mapped threadId input", async () => {
  const calls = [];
  const chatApi = createChatApi();
  const client = {
    mutation: async (fn, args) => {
...
test("createConvexPersistenceAdapter.listPendingServerRequests returns [] for missing thread read errors", async () => {
  const chatApi = createChatApi();
  const client = {
    mutation: async (fn) => {
