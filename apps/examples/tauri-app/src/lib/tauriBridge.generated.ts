...
export async function startBridge(config: StartBridgeConfig): Promise<unknown> {
  return await invoke("start_bridge", { config });
}

export async function sendUserTurn(text: string): Promise<unknown> {
...
export async function respondCommandApproval(config: { requestId: string | number; decision: CommandApprovalDecision }): Promise<unknown> {
  return await invoke("respond_command_approval", { config });
}

export async function respondFileChangeApproval(config: { requestId: string | number; decision: CommandApprovalDecision }): Promise<unknown> {
  return await invoke("respond_file_change_approval", { config });
}

export async function respondToolUserInput(config: { requestId: string | number; answers: Record<string, ToolUserInputAnswer> }): Promise<unknown> {
  return await invoke("respond_tool_user_input", { config });
}

export async function readAccount(config?: { refreshToken?: boolean }): Promise<unknown> {
  return await invoke("read_account", { config: config ?? {} });
}

export async function loginAccount(config: { params: LoginAccountParams }): Promise<unknown> {
  return await invoke("login_account", { config });
}

export async function cancelAccountLogin(config: { loginId: string }): Promise<unknown> {
...
export async function readAccountRateLimits(): Promise<unknown> {
  return await invoke("read_account_rate_limits");
}

export async function respondChatgptAuthTokensRefresh(config: { requestId: string | number; accessToken: string; chatgptAccountId: string; chatgptPlanType?: string | null }): Promise<unknown> {
  return await invoke("respond_chatgpt_auth_tokens_refresh", { config });
}

export async function setDisabledTools(config: { tools: string[] }): Promise<unknown> {
  return await invoke("set_disabled_tools", { config });
}

export async function stopBridge(): Promise<unknown> {
