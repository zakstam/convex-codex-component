...
import { useCodexElectronEvents, type PendingAuthRefreshRequest } from "./hooks/useCodexElectronEvents";
import { KNOWN_DYNAMIC_TOOLS, ELECTRON_RUNTIME_TOOL_PROMPT } from "./lib/dynamicTools";

const ACTOR_STORAGE_KEY = "codex-local-electron-actor-user-id";
...
function resolveActorUserId(): string {
  ...
  while (true) {
    ...
    if (entered === undefined) {
      throw new Error("Username is required to start the Electron Codex host.");
    }
  }
}
...
function AppContent({
  ...
  const runtimeBridgeControls = useMemo(
    () => ({
      start: electronBridge.lifecycle.start,
      stop: electronBridge.lifecycle.stop,
      getState: electronBridge.lifecycle.getState,
      sendTurn: electronBridge.turns.send,
      interrupt: electronBridge.turns.interrupt,
    }),
    [],
  );
  ...
  const accountAuth = useCodexAccountAuth<LoginAccountParams>({
    readAccount: electronBridge.account.read,
    loginAccount: electronBridge.account.login,
    cancelAccountLogin: electronBridge.account.cancelLogin,
    logoutAccount: electronBridge.account.logout,
    readAccountRateLimits: electronBridge.account.readRateLimits,
    respondChatgptAuthTokensRefresh: electronBridge.account.respondChatgptAuthTokensRefresh,
  });
  ...
  const conversation = useCodex({
    ...
    composer: {
      onSend: async (text: string) => {
        try {
          await electronBridge.turns.send(text);
  ...
  const onInsertDynamicToolPrompt = () => {
    const prompt = ELECTRON_RUNTIME_TOOL_PROMPT;
  ...
  const onSetDisabledTools = async (nextTools: string[]) => {
    ...
    try {
      await electronBridge.tools.setDisabled({ tools: normalized });
      setBridge((current) => ({
        ...current,
  ...
  const onRespondCommandOrFile = async (
    ...
    try {
      if (request.method === "item/commandExecution/requestApproval") {
        await electronBridge.approvals.respondCommand({ requestId: request.requestId, decision });
      } else {
        await electronBridge.approvals.respondFileChange({ requestId: request.requestId, decision });
      }
      setBridge((prev) => ({ ...prev, lastError: null }));
  ...
  const onRespondToolUserInput = async (request: PendingServerRequest) => {
    ...
    setSubmittingRequestKey(key);
    try {
      await electronBridge.approvals.respondToolInput({ requestId: request.requestId, answers });
      setBridge((prev) => ({ ...prev, lastError: null }));
  ...
  const onDeleteCurrentThread = async () => {
    ...
    try {
      const result = await deleteThreadCascadeMutation({
        ...
        reason: "electron-ui-delete-thread",
  ...
  const onDeleteLatestTurn = async () => {
    ...
    try {
      const result = await deleteTurnCascadeMutation({
        ...
        reason: "electron-ui-delete-turn",
        delayMs: DEFAULT_DELETE_DELAY_MS,
      });
      setActiveDeletionJobId(result.deletionJobId);
  ...
  const onPurgeActorData = async () => {
    ...
    try {
      const result = await purgeActorDataMutation({
        actor,
        reason: "electron-ui-purge-actor",
        delayMs: DEFAULT_DELETE_DELAY_MS,
      });
      setActiveDeletionJobId(result.deletionJobId);
