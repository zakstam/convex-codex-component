...
import type { BridgeState } from "../lib/electronBridge";
...
type UseCodexElectronEventsOptions = {
...
export function useCodexElectronEvents({
  setBridge,
  setRuntimeLog,
  setAuthSummary,
  ...
  useEffect(() => {
    ...
    const offBridgeState = window.electronCodex.on("codex:bridge_state", (...args: unknown[]) => {
      const payload = args[0] as BridgeStateEvent;
    ...
    unsubs.push(offBridgeState);

    const offEvent = window.electronCodex.on("codex:event", (...args: unknown[]) => {
      const payload = args[0] as { kind: string; turnId?: string; threadId?: string };
      const line = `${payload.kind} (${payload.turnId ?? "-"})`;
      const id = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      setRuntimeLogRef.current((prev) => [{ id, line }, ...prev].slice(0, 8));
    });
    unsubs.push(offEvent);

    const offProtocolError = window.electronCodex.on("codex:protocol_error", (...args: unknown[]) => {
      const payload = args[0] as { message: string };
      setBridgeRef.current((prev) => ({ ...prev, lastError: payload.message }));
      console.error("[codex:protocol_error]", payload.message, payload);
      addToastRef.current("error", payload.message);
    });
    unsubs.push(offProtocolError);

    const offGlobalMessage = window.electronCodex.on("codex:global_message", (...args: unknown[]) => {
