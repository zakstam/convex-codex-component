...
async function main() {
  ...
  const actor = {
    ...
    deviceId: process.env.ACTOR_DEVICE_ID ?? "persistent-cli-wiring-smoke",
  };
  ...
  console.log("persistent-cli wiring smoke passed");
}

main().catch((error) => {
  const reason = error instanceof Error ? error.message : String(error);
  console.error("persistent-cli wiring smoke failed: " + reason);
  process.exit(1);
});
