...
const ACTOR_LOCK_TABLE = "electron_actor_lock";
const ACTOR_LOCK_ENABLED = process.env.ELECTRON_ACTOR_LOCK?.trim() === "1";

function requireIncomingUserId(actor: HostActorContext): string {
  const incomingUserId = actor.userId?.trim();
  if (!incomingUserId) {
    throw new Error("actor.userId is required for electron host API calls.");
  }
