...
1. Mount `@zakstam/codex-runtime/convex.config`.
2. Define host definitions via `defineCodexHostDefinitions(...)` from `@zakstam/codex-runtime-convex/host`.
3. Export explicit Convex `query/mutation` handlers from `convex/chat.ts`.
4. Start runtime with `createCodexHostRuntime(...)` from `@zakstam/codex-runtime/host`:
   - omit `persistence` for Codex-authoritative runtime-only mode.
   - pass `persistence` adapter for external durable persistence.
5. Build UI with hooks from `@zakstam/codex-runtime-react`.
...
| `HostRuntimePersistence` | `type` | Core persistence adapter contract for runtime integration. |

## `@zakstam/codex-runtime-convex`

| `API` | `Type` | `Notes` |
| --- | --- | --- |
| `createConvexPersistenceAdapter` | `function` | Convex-backed persistence adapter for host runtime. |
...
| `API` | `Type` | `Notes` |
| --- | --- | --- |
| `createTauriBridgeClient` | `function` | Typed Tauri bridge client for lifecycle, turns, account auth, and sync hydration flows. |
| `generateTauriArtifacts` | `function` | Generates Rust command/dispatch/invoke-handler and permission artifacts from canonical bridge metadata. |
| `TAURI_BRIDGE_COMMANDS` | `const` | Canonical command manifest for helper and artifact generation. |

## `@zakstam/codex-runtime-bridge-tauri/local-adapter`

| `API` | `Type` | `Notes` |
| --- | --- | --- |
| `CodexLocalBridge` | `class` | Node-only local CLI bridge implementation for runtime `bridgeFactory` wiring. |
...
## `@zakstam/codex-runtime/protocol`
...
- Runtime factory is adapter-first:
  - `createCodexHostRuntime({ ... })` (default codex-only persistence behavior)
  - `createCodexHostRuntime({ persistence, ... })` (external persistence adapter)
  - Node bridge creation is explicit via `bridgeFactory` (for example `@zakstam/codex-runtime-bridge-tauri`).
...
- Runtime start contract is split:
  ...
  - `importLocalThreadToPersistence({ runtimeThreadHandle, conversationId? })`: canonical single-call local-thread import into persistence when the configured adapter supports durable sync
...
- `importLocalThreadToPersistence(...)` fail-closes with `E_PERSISTENCE_DISABLED` when configured adapter does not provide durable sync APIs.
...
- `@zakstam/codex-runtime/test` exports `register` and `schema` for component-oriented test setup.
- For full implementation sequence, use `docs/CANONICAL_INTEGRATION.md`.
- For fail-fast setup diagnostics, run `pnpm --filter @zakstam/codex-runtime run doctor:integration`.
