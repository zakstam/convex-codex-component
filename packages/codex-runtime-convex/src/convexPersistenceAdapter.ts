...
import {
  ...
  type ActorContext,
  type IngestDelta,
} from "@zakstam/codex-runtime/host/contracts";
import { hasRecoverableIngestErrors } from "@zakstam/codex-runtime/host/ingest-recovery";
...
export type ConvexPersistenceAdapterChatApi = {
...
export type ConvexPersistenceAdapterOptions = {
  runtimeOptions?: {
...
export function createConvexPersistenceAdapter(
  client: ConvexHttpClientLike,
  chatApi: ConvexPersistenceAdapterChatApi,
  options?: ConvexPersistenceAdapterOptions,
): HostRuntimePersistence & { activeSessionId: string | null } {
  let activeSessionId: string | null = null;
  const dispatchQueues = new Map<string, DispatchQueueEntry[]>();
  const persistedConversationIdByPersistedThreadId = new Map<string, string>();
  const runtimeConversationIdByPersistedThreadId = new Map<string, string>();
  ...
  const persistence: HostRuntimePersistence = {
    ensureThread: async (args) => {
      const runtimeConversationId = args.runtimeConversationId ?? args.conversationId;
      ...
      persistedConversationIdByPersistedThreadId.set(opened.threadId, args.conversationId);
      runtimeConversationIdByPersistedThreadId.set(opened.threadId, runtimeConversationId);
      return {
    ...
    ensureSession: async (args) => {
      ...
      const conversationId = persistedConversationIdByPersistedThreadId.get(args.threadId) ?? args.threadId;
      const runtimeConversationId = runtimeConversationIdByPersistedThreadId.get(args.threadId) ?? conversationId;
    ...
    startConversationSyncSource: async (args) => {
      ...
      persistedConversationIdByPersistedThreadId.set(started.threadId, args.conversationId);
      if (args.runtimeConversationId !== undefined) {
        runtimeConversationIdByPersistedThreadId.set(started.threadId, args.runtimeConversationId);
      }
      return started;
    },
